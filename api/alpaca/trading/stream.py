...
class TradingStream:
    ...
    async def _connect(self):
        self._ws = await websockets_legacy.connect(self._endpoint, **self._websocket_params)
    ...
    async def _run_forever(self):
        ...
        while True:
            try:
                ...
                self._running = False
                log.warning(
                    "trading stream websocket error, restarting " + " connection: " + str(wse)
                )
            except Exception as e:
                log.exception("error during websocket " "communication: {}".format(str(e)))
            finally:
