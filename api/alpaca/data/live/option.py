...
class OptionDataStream(DataStream):
    ...
    def subscribe_snapshots(
        self, handler: Callable[[Union[OptionsSnapshot, Dict]], Awaitable[None]], *symbols: str
    ) -> None:
        """Subscribe to option snapshots (includes greeks, IV, and full market data)

        Args:
            handler (Callable[[Union[OptionsSnapshot, Dict]], Awaitable[None]]): The coroutine callback
                function to handle the incoming snapshot data.
            *symbols: List of option contract symbols to subscribe to. "*" for everything.
        """
        self._subscribe(handler, symbols, self._handlers["snapshots"])

    def unsubscribe_snapshots(self, *symbols: str) -> None:
        """Unsubscribe from snapshots

        Args:
            *symbols (str): List of option contract symbols to unsubscribe from. "*" for everything.
        """
