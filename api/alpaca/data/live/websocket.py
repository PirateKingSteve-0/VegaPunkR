...
class DataStream:
    ...
    async def _auth(self) -> None:
        ...
        r = await self._ws.recv()
        msg = msgpack.unpackb(r)
        log.debug(f"Auth response: {msg}")
        if msg[0]["T"] == "error":
            error_msg = msg[0].get("msg", "auth failed")
            error_code = msg[0].get("code", "unknown")
            log.error(f"Auth error: {error_msg} (code: {error_code})")
            raise ValueError(f"{error_msg} (code: {error_code})")
        if msg[0]["T"] != "success" or msg[0]["msg"] != "authenticated":
            log.error(f"Unexpected auth response: {msg}")
    ...
    def _cast(self, msg: Dict) -> Union[BaseModel, RawData]:
        ...
        if msg_type == "snapshot":
            return OptionsSnapshot(msg["S"], msg)
        return msg
